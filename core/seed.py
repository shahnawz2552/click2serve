...
def seed_default_admin_if_empty() -> None:
    """Create the default admin if no users exist. Imported lazily to avoid
    a circular import between db.py and auth.py."""
    from .auth import DEFAULT_ADMIN_PASSWORD, DEFAULT_ADMIN_USERNAME, hash_password
    ...
    with get_conn() as conn:
        ...
        conn.execute(
            ...
            "VALUES (?, ?, 'owner', ?)",
            (DEFAULT_ADMIN_USERNAME, hash_password(DEFAULT_ADMIN_PASSWORD), now),
        )
